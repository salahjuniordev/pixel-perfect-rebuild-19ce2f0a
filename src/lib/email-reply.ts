 ...
 *
 * Uses Resend (transactional email). RESEND_API_KEY lives only on the
 * server; every call is gated to an authenticated admin, same as the
 * AI suggest functions. Each sent reply is stored in `email_replies`
 * so the dashboard keeps a conversation history per submission.
 */

const FROM_DEFAULT = `${BRAND} <onboarding@resend.dev>`;
